...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        # if batch_idx > 0 and batch_idx < 141:
        #     continue
        ...
        for idx in range(len(sample['rgb'])):
            ...
            depth_img, uv_input, pc_input_valid = lidar_project_depth(pc_rotated, sample['calib'][idx],
                                                                      real_shape)  # image_shape
        ...
        for i in range(0, len(sample['rgb'])):
            ...
            RT1 = RTs[0][i]
            mis_calib = torch.tensor(sample['initial_RT'][i])[1:]
            ...
            log_string += [str(errors_t[0][i]), str(errors_r[0][i]), str(errors_t2[0][i][0].item()),
                           str(errors_t2[0][i][1].item()), str(errors_t2[0][i][2].item()),
                           str(errors_rpy[0][i][0].item()), str(errors_rpy[0][i][1].item()),
                           str(errors_rpy[0][i][2].item())]
        start = 0
        # Run model
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                if _config['iterative_method'] == 'single_range' or _config['iterative_method'] == 'single':
                    T_predicted_batch, R_predicted_batch = models[0](rgb_resize, lidar_resize)
                elif _config['iterative_method'] == 'multi_range':
                    T_predicted_batch, R_predicted_batch = models[iteration](rgb_resize, lidar_resize)
                run_time = time.time() - t1

                for i in range(len(sample['rgb'])):
                    # Project the points in the new pose predicted by the i-th network
                    R_predicted = quat2mat(R_predicted_batch[i])
                    T_predicted = tvector2mat(T_predicted_batch[i])
                    RT_predicted = torch.mm(T_predicted, R_predicted)
                    RTs[iteration + 1].append(torch.mm(RTs[iteration][i], RT_predicted))  # inv(H_gt)*H_pred_1*H_pred_2*.....H_pred_n
                    if iteration == 0:
                    ...
                    # depth_img_pred, uv_pred, pc_pred_valid = lidar_project_depth(rotated_point_cloud, \
                    #                                                              sample['calib'][i], \
                    #                                                              real_shape_input[i])  # image_shape
                    ...
                    log_string += [str(errors_t[iteration + 1][i]), str(errors_r[iteration + 1][i]),
                                   str(errors_t2[iteration + 1][i][0].item()),
                                   str(errors_t2[iteration + 1][i][1].item()),
                                   str(errors_t2[iteration + 1][i][2].item()),
                                   str(errors_rpy[iteration + 1][i][0].item()),
                                   str(errors_rpy[iteration + 1][i][1].item()),
                                   str(errors_rpy[iteration + 1][i][2].item())]
    ...
    mis_calib_input = torch.stack(mis_calib_list)
